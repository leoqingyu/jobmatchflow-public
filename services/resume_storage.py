"""定制简历 DOCX 单文件落盘：每个 user+job 仅一份，更新时删除旧文件。"""
from __future__ import annotations

from pathlib import Path

from core.config import settings
from core.exceptions import RenderError
from core.logger import get_logger

logger = get_logger(__name__)

RESUME_SUBDIR = "resumes"


def _resume_docx_path(user_id: int, job_id: int) -> Path:
    root = Path(settings.local_storage_base_path).resolve()
    d = root / RESUME_SUBDIR
    d.mkdir(parents=True, exist_ok=True)
    return d / f"u{user_id}_j{job_id}.docx"


def remove_file_if_exists(path: str | None) -> None:
    if not path:
        return
    try:
        p = Path(path)
        if p.is_file():
            p.unlink()
            logger.info("已删除旧文件: %s", path)
    except OSError as e:
        logger.warning("删除文件失败 %s: %s", path, e)


def write_resume_docx(docx_bytes: bytes, user_id: int, job_id: int, previous_path: str | None) -> str:
    """写入（覆盖）当前 user+job 的 DOCX。若 previous_path 指向另一路径则先删。"""
    if not docx_bytes:
        raise RenderError("DOCX 内容为空，无法写入")
    target = _resume_docx_path(user_id, job_id)
    if previous_path:
        prev = Path(previous_path).resolve()
        if prev.is_file() and prev != target.resolve():
            remove_file_if_exists(previous_path)
    target.write_bytes(docx_bytes)
    return str(target.resolve())
